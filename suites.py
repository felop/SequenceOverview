...
def init():
    ...
    for LineX in range(input_scale[0]+1):
        pygame.draw.line(screen, grid_color, (LineX*(input_size[0]/input_scale[0]),0), (LineX*(input_size[0]/input_scale[0]),input_size[1]))
        if LineX <= input_scale[0]:
            val_x = text_font.render(str(int(LineX-input_scale[0]/2)), True, text_color)
            screen.blit(val_x, (LineX*(input_size[0]/input_scale[0]) -12,input_size[1]/2 +5))

    for LineY in range(input_scale[1]+1):
        pygame.draw.line(screen, grid_color, (0,LineY*(input_size[1]/input_scale[1])), (input_size[0],LineY*(input_size[1]/input_scale[1])))
        if LineY <= input_scale[1]:
            val_y = text_font.render(str(-int(LineY-input_scale[1]/2)), True, text_color)
            screen.blit(val_y, (input_size[0]/2 -12, LineY*(input_size[1]/input_scale[1]) +5))

    axes_color = (100,255,0)
    pygame.draw.line(screen, axes_color, (input_size[0]/2,0), (input_size[0]/2,input_size[1]))
    ...
    pygame.draw.rect(screen, (50,50,50), graph)
...
def update():
    ...
    for val_x in range(input_size[0]):
        ...
        if defined:
            pygame.gfxdraw.pixel(screen, val_x, int((-val_y+input_scale[0]/2)*input_size[0]/input_scale[0]), func_color)
    ...
    if defined:
        while math.sqrt((image[0]-mirrored[0])**2+ (image[1]-mirrored[1])**2) > 3 and boundaries(image) and boundaries(mirrored):
            ...
            pygame.draw.line(screen, u1_line_color, (int(image[0]),int(image[1])), (int(mirrored[0]),int(mirrored[1])))
            prev_mirrored = mirrored
            if i == 0:
                pygame.draw.line(screen, (255,255,255), (image[0]-3,image[1]), (image[0]+3,image[1]))
                pygame.draw.line(screen, (255,255,255), (image[0],image[1]-3), (image[0],image[1]+3))
            i += 1
            image, mirrored,_ = step(prev_mirrored[0])

    pointer_color = (255,255,255)
    pygame.draw.line(screen, pointer_color, (mouse_pos[0]-5,mouse_pos[1]), (mouse_pos[0]+5,mouse_pos[1]))
    pygame.draw.line(screen, pointer_color, (mouse_pos[0],mouse_pos[1]-5), (mouse_pos[0],mouse_pos[1]+5))
    ...
    if defined:
        ...
        if output_scale[0] != output_scale[1]:
            for i in range(len(first_images)):
                ...
                pygame.draw.line(screen, u1_line_color, (x-5, y), (x+5, y))
                pygame.draw.line(screen, u1_line_color, (x, y-5), (x, y+5))
                u_text = text_font.render("U"+str(i), True, u0_line_color)
                screen.blit(u_text, (x-17,y+2))
                u_text = text_font.render(str(round(first_images[i],3)), True, (200,200,200))
